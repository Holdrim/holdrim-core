...
# is gone and not a zombie the moment this script says so — but only for so long: a JVM stuck in a
# shutdown hook would otherwise hold the session start forever, so it gets KILL after five seconds.
