...
| **dependency** | block B stands on block A. Declared by hand, in `data-depends` — `holdrim propose-deps` (issue #43) proposes a likely one, deterministically, from two blocks sharing a term of the PROJECT's own `content.glossary` (`holdrim.json`, never this file's own vocabulary); a proposal is a `request` like any other, and only an approved one is ever added by hand |
...
| **agent token** | an agent's credential of its own, issued and revoked by the owner alone on the people screen: one per address, never expiring until revoked, shown once and stored as a hash. Whoever comes in with it is an agent (`roles.isAgent`), whatever its address, and reaches only the API's event routes — never a ✓, triage, a lock or people. The CLI writes with it (`HOLDRIM_AGENT_TOKEN`). `docs/ROLES.md` section 4 |
...
| **source** | where the agent's tool reads events from: the cloud, a SQLite file (`--db`), or a local server (`--local`) — and where it writes them: the server's API, always. `engine/cli/remote.ts` |
...
| `dependency` | a likely `data-depends`, proposed deterministically by `holdrim propose-deps` from two blocks sharing a term of the project's own `content.glossary` (`engine/cli/propose.ts`, issue #43) — a request like any other; approving one does not write the `data-depends` itself |
